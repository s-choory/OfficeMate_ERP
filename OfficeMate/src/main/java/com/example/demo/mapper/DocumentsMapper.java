...
public interface DocumentsMapper {
	...
	@Select("WITH LatestDocuments AS ("
			...
			+ "    FROM documents" + ") " + "SELECT count(*) FROM LatestDocuments " + "WHERE version = 1")
	int documentsGetCount();

	@Select("WITH LatestDocuments AS ("
			+ "    SELECT *, ROW_NUMBER() OVER (PARTITION BY documentId ORDER BY version DESC) as rn "
			+ "    FROM documents" + ") " + "SELECT * FROM LatestDocuments " + "WHERE version = 1"
			+ "ORDER BY documentId DESC " + "LIMIT #{rowCount} OFFSET #{offset}")
	List<DocumentDTO> getListAdminPage(PageDTO pageDTO);

	@Select("WITH LatestDocuments AS ("
			+ "    SELECT *, ROW_NUMBER() OVER (PARTITION BY documentId ORDER BY version DESC) as rn "
			+ "    FROM documents" + ") " + "SELECT count(*) FROM LatestDocuments "
			+ "WHERE version = 1 AND (uploadUser = #{name} OR shareUser = #{name}) ")
	int documentgetCount(String name);

	@Select("WITH LatestDocuments AS ("
			+ "    SELECT *, ROW_NUMBER() OVER (PARTITION BY documentId ORDER BY version DESC) as rn "
			+ "    FROM documents" + ") " + "SELECT * FROM LatestDocuments "
			+ "WHERE version = 1 AND (uploadUser = #{name} OR shareUser = #{name}) " + "ORDER BY documentId DESC "
